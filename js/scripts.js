...
function getOrCreatePavementPatternEl(polygon) {
...
// <pattern patternUnits="userSpaceOnUse"> with no x/y tiles starting from
// (0,0) of whatever local pixel space the referencing path happens to be
// drawn in -- and Leaflet silently re-bases that pixel space (its "pane
// origin") on every zoom change to keep coordinates from overflowing far
// from the map center. The polygon's own path gets redrawn relative to the
// new origin, but the pattern's phase has no memory of the old one, so a
// naively-drawn pattern visibly jumps on each zoom. Fix: anchor the
// pattern's phase to a fixed point on the polygon itself (its first
// vertex) instead, recomputed fresh on every redraw via
// latLngToLayerPoint() -- the same pixel space Leaflet uses for the
// polygon's own path "d" attribute -- so the fill stays visually locked to
// the polygon (and to the ground) regardless of zoom/pan. Returns the
// anchor projected onto the pattern's own pre-rotation local axes (undoing
// the rotation that patternTransform applies), since that's the frame
// spacing/offset controls operate in.
function getPavementPatternAnchor(polygon, rotationDeg) {
    const latlngs = polygon.getLatLngs()[0] || polygon.getLatLngs();
    const anchorPoint = map.latLngToLayerPoint(latlngs[0]);
    const rad = -rotationDeg * Math.PI / 180;
    return {
        x: anchorPoint.x * Math.cos(rad) - anchorPoint.y * Math.sin(rad),
        y: anchorPoint.x * Math.sin(rad) + anchorPoint.y * Math.cos(rad)
    };
}

// Rebuilds/updates the <pattern> element backing a pattern-filled polygon,
// dispatching to the renderer for its current pattern type. Safe to call
// repeatedly -- both renderers reuse the polygon's existing <pattern>
// element if there is one instead of recreating it.
function updatePavementPattern(polygon) {
    const style = polygon.myStyle;
    if (!style || style.fillMode !== 'pattern') return;
    if (style.pattern === 'custom-svg') {
        updateCustomSvgPattern(polygon);
    } else {
        updateStripePattern(polygon);
    }
}

// Renders the "Stripes (Crosswalk)" pattern: parallel lines at a given
// width/spacing/rotation/offset (all in decimal feet, converted to pixels
// at the current zoom).
function updateStripePattern(polygon) {
    const style = polygon.myStyle;
    ...
    const anchor = getPavementPatternAnchor(polygon, rotation);
    const offsetPx = feetToPavementPatternPixels(style.patternOffsetFt || 0);
    // Wrap into a single tile's width -- this can land anywhere in
    // [0, spacing), including right at an edge.
    const phaseX = (((anchor.x + offsetPx) % spacing) + spacing) % spacing;

    const patternEl = getOrCreatePavementPatternEl(polygon);
    ...
    // <pattern> content is clipped to its own [0, spacing) tile box by
    // default, so a stripe positioned near a tile edge (which phaseX can
    // land on, depending on the anchor/offset) would have part of its
    // stroke-width padding cut off. Drawing the same stripe at phaseX and
    // at one tile-width to either side guarantees a full, unclipped stripe
    // regardless of where phaseX falls within the tile.
    const stripeAt = x => `<line x1="${x}" y1="0" x2="${x}" y2="${spacing}" stroke="${color}" stroke-width="${strokeWidth}" />`;
    patternEl.innerHTML = stripeAt(phaseX - spacing) + stripeAt(phaseX) + stripeAt(phaseX + spacing);
}

// Renders the "Custom SVG" pattern: the user's own (sanitized) SVG artwork,
// tiled at a given real-world size/rotation/offset. Unlike stripes, an
// arbitrary graphic has no translation symmetry along either axis, so both
// X and Y offsets matter (stripes only expose one, since sliding parallel
// lines along their own length has no visible effect).
function updateCustomSvgPattern(polygon) {
    const style = polygon.myStyle;
    ...
    const anchor = getPavementPatternAnchor(polygon, rotation);
    const offsetXPx = feetToPavementPatternPixels(style.customSvgOffsetXFt || 0);
    const offsetYPx = feetToPavementPatternPixels(style.customSvgOffsetYFt || 0);
    const phaseX = (((anchor.x + offsetXPx) % spacing) + spacing) % spacing;
    const phaseY = (((anchor.y + offsetYPx) % spacing) + spacing) % spacing;
    ...
    // tile would be partly cut off. Unlike the 1D stripe pattern, an
    // arbitrary 2D graphic needs the full 3x3 grid of neighboring copies
    // to guarantee nothing is clipped regardless of where the phase lands.
    let inner = '';
...
function applyPavementFill(polygon) {
    const style = polygon.myStyle;
    if (!style) return;
    if (style.fillMode === 'pattern' && style.pattern === 'custom-svg' && !style.customSvgMarkup) {
        // Custom SVG selected but no artwork loaded yet (via the paste
        // box/file upload) -- render as transparent rather than pointing
        // fillColor at a <pattern> def that doesn't exist yet, which
        // browsers fall back to solid black for.
        removePavementPattern(polygon);
        polygon.setStyle({ fillColor: 'none', fillOpacity: 0 });
...
// The pattern's anchor point (see updatePavementPattern()) is the
// polygon's first vertex, so moving or reshaping the polygon needs to
// re-anchor the pattern too -- otherwise it stays visually "in place" while
// the polygon moves over it until the next zoom change happens to trigger
// a refresh. Call this once per polygon right after it's created.
function attachPatternDragRefresh(polygon) {
    polygon.on('editable:vertex:dragend editable:dragend', function () {
...
// Pattern tiles are sized/anchored in on-screen pixels, so refresh every
// pattern-filled polygon whenever the zoom level changes (tile size) or the
// map's pane origin gets reset (viewreset -- can also happen from panning
// far enough, not just zooming; this is what keeps the pattern's anchor
// point, see updatePavementPattern(), in sync with the newly-rebased pixel
// space Leaflet just switched to).
...
function syncPavementFillControlsToPolygon(polygon) {
    ...
    const isCustomSvg = isPattern && style.pattern === 'custom-svg';
    ...
    document.getElementById('pavementStripeControls').style.display = isCustomSvg ? 'none' : 'block';
    document.getElementById('pavementSvgControls').style.display = isCustomSvg ? 'block' : 'none';
    ...
    const offsetFt = style.patternOffsetFt || 0;
    document.getElementById('pavementPatternOffset').value = offsetFt;
    document.getElementById('pavementPatternOffsetValue').textContent = offsetFt.toFixed(2) + " ft";

    document.getElementById('pavementSvgPaste').value = style.customSvgMarkup || '';
    ...
    document.getElementById('pavementSvgOffsetXValue').textContent = svgOffsetXFt.toFixed(2) + " ft";

    const svgOffsetYFt = style.customSvgOffsetYFt || 0;
    document.getElementById('pavementSvgOffsetY').value = svgOffsetYFt;
    document.getElementById('pavementSvgOffsetYValue').textContent = svgOffsetYFt.toFixed(2) + " ft";
}

// Parses and sanitizes a user-supplied SVG document string for safe use as
// pattern-fill content. This can come from an uploaded file or pasted text
// the user didn't necessarily author themselves, and SVG can carry
// <script> elements and "on*" event-handler attributes that execute
// arbitrary JS in the page -- both are stripped. Returns null if the text
// doesn't parse as SVG at all.
function sanitizeSvgMarkup(svgText) {
    ...
    do {
        ...
        if (node.attributes) {
            [...node.attributes].forEach(attr => {
                ...
                if (name.startsWith('on') || (/^(href|xlink:href)$/.test(name) && /^\s*javascript:/i.test(attr.value))) {
    ...
    // Fall back to width/height (or a generic square) if there's no
    // viewBox, so the artwork still has a defined aspect ratio to scale by.
    const viewBox = viewBoxAttr || `0 0 ${width || 100} ${height || 100}`;

    return { markup: svgEl.innerHTML, viewBox: viewBox };
}

// Parses whatever's currently in the SVG paste box (populated directly by
// a file upload, or typed/pasted by hand) and applies it to the selected
// polygon, or just validates it if nothing is selected yet -- mirroring
// how the other pattern controls double as "pending settings" that get
// picked up by the next New Polygon Shape.
function applyPavementSvgFromTextarea() {
    const statusEl = document.getElementById('pavementSvgStatus');
    const text = document.getElementById('pavementSvgPaste').value;
    const parsed = sanitizeSvgMarkup(text);
    if (!parsed) {
        statusEl.textContent = 'Could not parse that as SVG.';
        statusEl.style.color = '#c00';
        return;
    }
    statusEl.textContent = 'SVG loaded.';
    statusEl.style.color = '#080';
    if (selectedPavementPolygon && selectedPavementPolygon.myStyle.pattern === 'custom-svg') {
        selectedPavementPolygon.myStyle.customSvgMarkup = parsed.markup;
        selectedPavementPolygon.myStyle.customSvgViewBox = parsed.viewBox;
        applyPavementFill(selectedPavementPolygon);
    }
}

document.getElementById('pavementSvgFile').addEventListener('change', function (e) {
    const file = e.target.files[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = function (evt) {
        document.getElementById('pavementSvgPaste').value = evt.target.result;
        applyPavementSvgFromTextarea();
    };
    reader.readAsText(file);
});

document.getElementById('pavementSvgApply').addEventListener('click', applyPavementSvgFromTextarea);
...
document.getElementById('drawPavementPolygon').addEventListener('click', function () {
    ...
    // setStyle below) ensures the polygon actually renders in pavementPane,
    // underneath the lane lines/markers/signs, instead of Leaflet's default
    // overlayPane on top of them.
    var polygon = map.editTools.startPolygon(null, { pane: 'pavementPane' });
    polygon.on('editable:drawing:end', function () {
        ...
        attachPatternDragRefresh(polygon);
        pavementPolygonsLayer.addLayer(polygon);
...
document.getElementById('duplicatePavementPolygon').addEventListener('click', function () {
    ...
    attachPatternDragRefresh(duplicate);
...
document.getElementById('pavementFillType').addEventListener('change', function () {
    ...
    // Switching pattern type resets its controls to defaults, both in the
    // form and, if selected, on the polygon itself.
    const defaults = PAVEMENT_PATTERN_DEFAULTS.stripes;
    ...
    document.getElementById('pavementPatternOffset').value = 0;
    document.getElementById('pavementPatternOffsetValue').textContent = "0.00 ft";
    ...
    if (selectedPavementPolygon) {
        ...
        if (isPattern && !isCustomSvg) {
            ...
            selectedPavementPolygon.myStyle.patternOffsetFt = 0;
        }
        applyPavementFill(selectedPavementPolygon);
    }
    // Custom SVG needs its own artwork (from the paste box/file upload)
    // before there's anything to render -- syncPavementFillControlsToPolygon()
    // already restores it when re-selecting a polygon that has one, and the
    // "Use This SVG" button/file input apply it explicitly otherwise.
});
...
document.getElementById('pavementPatternOffset').addEventListener('input', function () {
    const offsetFt = parseFloat(this.value) || 0;
    document.getElementById('pavementPatternOffsetValue').textContent = offsetFt.toFixed(2) + " ft";
    if (selectedPavementPolygon) {
        selectedPavementPolygon.myStyle.patternOffsetFt = offsetFt;
        applyPavementFill(selectedPavementPolygon);
    }
});
...
document.getElementById('pavementSvgOffsetX').addEventListener('input', function () {
    const offsetFt = parseFloat(this.value) || 0;
    document.getElementById('pavementSvgOffsetXValue').textContent = offsetFt.toFixed(2) + " ft";
    if (selectedPavementPolygon) {
        selectedPavementPolygon.myStyle.customSvgOffsetXFt = offsetFt;
        applyPavementFill(selectedPavementPolygon);
    }
});

document.getElementById('pavementSvgOffsetY').addEventListener('input', function () {
    const offsetFt = parseFloat(this.value) || 0;
    document.getElementById('pavementSvgOffsetYValue').textContent = offsetFt.toFixed(2) + " ft";
    if (selectedPavementPolygon) {
        selectedPavementPolygon.myStyle.customSvgOffsetYFt = offsetFt;
        applyPavementFill(selectedPavementPolygon);
    }
});
