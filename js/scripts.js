...
// current style (pattern type, rotation, line width/spacing/offset, color)
// and the current zoom/position. Safe to call repeatedly -- it reuses the
// polygon's existing <pattern> element if there is one instead of
// recreating it.
function updatePavementPattern(polygon) {
    ...
    // <pattern patternUnits="userSpaceOnUse"> with no x/y tiles starting
    // from (0,0) of whatever local pixel space the referencing path happens
    // to be drawn in -- and Leaflet silently re-bases that pixel space
    // (its "pane origin") on every zoom change to keep coordinates from
    // overflowing far from the map center. The polygon's own path gets
    // redrawn relative to the new origin, but the pattern's phase has no
    // memory of the old one, so the stripes visibly jump on each zoom.
    // Fix: anchor the pattern's phase to a fixed point on the polygon
    // itself (its first vertex) instead, recomputed fresh on every redraw
    // via latLngToLayerPoint() -- the same pixel space Leaflet uses for the
    // polygon's own path "d" attribute -- so the stripes stay visually
    // locked to the polygon (and to the ground) regardless of zoom/pan.
    const latlngs = polygon.getLatLngs()[0] || polygon.getLatLngs();
    const anchorPoint = map.latLngToLayerPoint(latlngs[0]);

    // Project the anchor onto the pattern's own pre-rotation local x-axis
    // (the axis spacing/offset run along) by undoing the rotation applied
    // below, so the anchor lines up correctly regardless of the pattern's
    // current rotation.
    const rad = -rotation * Math.PI / 180;
    const anchorLocalX = anchorPoint.x * Math.cos(rad) - anchorPoint.y * Math.sin(rad);
    ...
    const phaseX = (((anchorLocalX + offsetPx) % spacing) + spacing) % spacing;
...
document.getElementById('pavementFillType').addEventListener('change', function () {
    ...
    // Switching pattern type resets Line Width/Spacing/Offset to that
    // pattern's defaults, both in the controls and, if selected, on the
    // polygon itself.
    const defaults = PAVEMENT_PATTERN_DEFAULTS.stripes;
